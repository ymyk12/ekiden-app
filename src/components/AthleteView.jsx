  ...
  return (
    ...
    >
      ...
      {isMenuOpen && (
        ...
        >
          <div
            className="bg-white w-full max-w-sm p-6 rounded-[2.5rem] space-y-4 shadow-2xl animate-in zoom-in-95"
      ...
      <header
        className={`sticky top-0 z-50 bg-blue-600 text-white pt-12 pb-4 px-6 rounded-b-[2.5rem] relative overflow-hidden shadow-lg shadow-blue-900/20 ${isPreview ? "mt-8" : ""}`}
      >
        ...
        <div className="flex justify-between items-center relative z-10 max-w-md mx-auto">
          ...
          <div className="text-center">
            <p className="text-blue-100 text-[10px] font-black tracking-widest uppercase mb-0.5">
              Athlete Dashboard
            </p>
        ...
        {availablePeriods.length > 0 && selectedPeriod && (
          <div className="mt-3 max-w-md mx-auto relative z-10">
            <div className="bg-white/20 backdrop-blur-md p-2 rounded-2xl flex items-center gap-2 border border-white/30">
              <Filter size={14} className="text-white ml-2 opacity-80 flex-shrink-0" />
              <select
      ...
      <main className="px-5 space-y-6 relative z-20 max-w-md mx-auto">
          ...
          <>
            ...
            {hasUnreadReview && (
              <div
                ...
                className="bg-gradient-to-r from-indigo-600 to-blue-600 text-white p-4 rounded-2xl shadow-lg mb-6 flex items-center justify-between cursor-pointer active:scale-95 transition-transform animate-in slide-in-from-top-2"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="font-bold text-sm truncate">
                      {currentFeedback?.coachComment
                        ? `「${currentFeedback.coachComment.slice(0, 20)}${currentFeedback.coachComment.length > 20 ? "…" : ""}」`
                        : "監督からフィードバックが届いています"}
                    </p>
                  </div>
                </div>
                <div className="bg-white text-indigo-600 px-3 py-1.5 rounded-lg text-xs font-black shadow-sm flex-shrink-0 ml-2">
                  確認する
                </div>
              </div>
            )}
              ...
              {activeQuarters &&
                ...
                targetPeriod.type !== "month" && (
                  <div className="pt-4 border-t border-slate-100">
                    <button
                      onClick={() => setIsQuarterExpanded((v) => !v)}
                    ...
                    >
                      ...
                      <ChevronDown
                        size={14}
                        className={`text-slate-400 transition-transform duration-200 ${isQuarterExpanded ? "rotate-180" : ""}`}
                      />
                    </button>
                    {isQuarterExpanded && (
            ...
            {(() => {
              ...
              return (
                <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-5 rounded-[2.5rem] text-white shadow-xl shadow-slate-900/30 relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="w-full flex items-start gap-2">
                      <div className="flex-1 min-w-0">
                        <p className="text-blue-400 text-[10px] font-black uppercase tracking-widest flex items-center gap-2 mb-2">
                          ...
                          {diary.location && (
                            <span className="text-xs font-bold text-slate-300 flex items-center gap-1">
                              ...
                              {diary.location === "その他" && diary.locationDetail
                                ...
                                : diary.location === "競技場" && diary.locationDetail
                                  ? `${diary.location}(${diary.locationDetail})`
                                  : diary.location}
                            </span>
                          )}
                          {diary.startTime && (
                            <span className="text-[10px] font-bold bg-white/10 px-2 py-0.5 rounded-lg border border-white/10">
                              {diary.startTime}{diary.endTime ? ` - ${diary.endTime}` : ""}
                            </span>
                          )}
                          {diary.weather && (
                            <span className="text-[10px] font-bold bg-white/10 px-2 py-0.5 rounded-lg border border-white/10">
                        ...
                        <p className="font-bold text-sm leading-snug text-white/90">
                          {diary.menu}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 space-y-3">
                      {diary.reinforcements && diary.reinforcements.length > 0 && (
                        <div className="bg-blue-900/30 p-3 rounded-xl border border-blue-500/20">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {diary.reinforcements.map((item, i) => (
                              <span key={i} className="text-[10px] font-bold bg-blue-500/20 text-blue-100 px-2 py-1 rounded">
                                {item}
                              </span>
                            ))}
                            {diary.reinforcements.includes("その他") && diary.reinforcementDetail && (
                      ...
                      {diary.coachNote && (
                        <div className="bg-amber-300/20 p-3 rounded-xl border border-amber-300/30">
        ...
        {/* MY LOG */}
        {view === "diary" && (
          <div className="bg-white rounded-[3rem] shadow-sm animate-in fade-in flex flex-col h-[calc(100dvh-290px)]">
            {/* サブヘッダー（固定） */}
            <div className="flex items-center justify-between px-6 pt-6 pb-4 flex-shrink-0 border-b border-slate-100">
              <button onClick={() => safeChangeView("menu")} className="p-2 bg-slate-100 rounded-full text-slate-500 hover:bg-slate-200">
                <ArrowLeft size={20} />
              </button>
              <h3 className="font-black uppercase text-[10px] tracking-widest text-slate-400 tracking-[0.3em]">My Log</h3>
              <div className="w-9" />
            </div>
            {/* スクロールエリア */}
            <div className="flex-1 overflow-y-auto px-6 py-4 pb-8 space-y-3">
              {periodLogs.length === 0 ? (
                      ...
                      <div key={log.id} className="bg-slate-50 rounded-2xl border border-slate-100 px-4 py-3 flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex flex-col items-center justify-center text-white flex-shrink-0 ${isRest ? "bg-emerald-400" : "bg-blue-500"}`}>
                          {isRest ? (
                            ...
                            <>
                              <span className="text-[11px] font-black leading-none">{log.distance}</span>
                              <span className="text-[8px] font-bold leading-none mt-0.5">km</span>
                            </>
                          )}
                        </div>
        ...
        {view === "review" && (
          <div className="bg-white rounded-[3rem] shadow-sm animate-in fade-in flex flex-col h-[calc(100dvh-290px)]">
            {/* サブヘッダー（固定） */}
            <div className="flex items-center gap-2 px-6 pt-6 pb-4 flex-shrink-0 border-b border-slate-100">
              <button onClick={() => safeChangeView("diary")} className="p-2 bg-slate-100 rounded-full text-slate-500 hover:bg-slate-200">
                <ArrowLeft size={20} />
              </button>
              <h3 className="font-black uppercase text-[10px] tracking-widest text-slate-400 tracking-[0.3em]">Period Review</h3>
            </div>
            {/* スクロールエリア */}
          ...
          <div className="bg-white rounded-[3rem] shadow-sm animate-in fade-in flex flex-col h-[calc(100dvh-290px)]">
            ...
            <div className="flex-shrink-0 border-b border-slate-100">
              <div className="flex items-center gap-2 px-6 pt-6 pb-3">
                ...
                <h3 className="font-black uppercase text-[10px] tracking-widest text-slate-400 tracking-[0.3em]">Team</h3>
                <div className="w-9" />
              </div>
            {/* サブタブ */}
            <div className="flex gap-1 bg-slate-100 p-1 rounded-2xl">
              {[
                ...
                <button
                  key={tab.id}
                  onClick={() => setTeamSubView(tab.id)}
                  className={`flex-1 py-2 text-[11px] font-black rounded-xl transition-all ${teamSubView === tab.id ? "bg-white text-blue-600 shadow-sm" : "text-slate-400"}`}
                >
                  {tab.label}
                </button>
              ))}
                      ...
                      >
                        ...
                        <div className="text-right">
                          <p className="text-lg font-black text-slate-800">
                            {runner.total}
                            <span className="text-xs ml-0.5 text-slate-400">km</span>
                          </p>
                        </div>
                      </div>
                    );
                ...
                {(() => {
                  ...
                  return filteredLogs.map((log) => {
                    ...
                    return (
                      ...
                      >
                        <div className="px-4 pb-4 space-y-4 border-t border-slate-200/50 pt-4 animate-in fade-in">
                          <div className="grid grid-cols-2 gap-2 text-[10px] font-bold text-slate-500">
                            <div className="bg-white p-2 rounded-lg border border-slate-100">
                              <span className="text-indigo-400 block text-[9px] uppercase">Time</span>
                              {log.startTime ? `${log.startTime} - ${log.endTime}` : "-"}
                            </div>
                            <div className="bg-white p-2 rounded-lg border border-slate-100">
                              <span className="text-indigo-400 block text-[9px] uppercase">Cond</span>
                              {log.temp ? `${log.temp}℃` : "-"} / Wind: {log.wind || "-"}
                            </div>
                          </div>
                          {log.reinforcements && log.reinforcements.length > 0 && (
                            <div className="bg-blue-50/50 p-3 rounded-xl border border-blue-100">
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                {log.reinforcements.map((item, i) => (
                                  <span key={i} className="text-[10px] font-bold bg-white text-slate-600 px-2 py-1 rounded border border-blue-100">
                                    {item}
                                  </span>
                                ))}
              ...
              <div className="space-y-4 max-h-[500px] overflow-y-auto no-scrollbar pr-1">
                ...
                {(() => {
                  ...
                  return teamLogsList.map((log) => {
                    const isRest = log.category === "完全休養";
                    return (
                      <div key={log.id} className="flex items-start gap-3 relative pl-2">
                        <div className="absolute left-[19px] top-8 bottom-[-16px] w-0.5 bg-slate-100 last:hidden"></div>
                        <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center font-black text-xs text-white shadow-sm z-10 ${isRest ? "bg-emerald-400" : "bg-blue-500"}`}>
                        ...
                        <div className="bg-slate-50 p-3 rounded-2xl w-full border border-slate-100">
                          <div className="flex justify-between items-start">
                            <div>
                              <p className="text-[10px] font-bold text-slate-400">
                                {log.date.slice(5).replace("-", "/")} · {log.runnerName}
                              </p>
                              <p className={`text-sm font-black ${isRest ? "text-emerald-600" : "text-slate-700"}`}>
      ...
      {/* My Log 期間分析 FAB — diary ビューのみ表示 */}
      {view === "diary" && (
        <button
          onClick={() => safeChangeView("review")}
          className="fixed bottom-28 right-4 z-30 flex items-center gap-1.5 bg-blue-500 text-white text-[11px] font-black px-3.5 py-2.5 rounded-2xl shadow-lg shadow-blue-500/30 active:scale-95 transition-all"
        >
          <Activity size={13} />
          期間分析
        </button>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 max-w-md mx-auto pointer-events-none">
        <div className="bg-white/95 backdrop-blur-xl border-t border-slate-200 pb-8 pt-2 px-3 rounded-t-[2.5rem] shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] flex justify-between items-end h-24 pointer-events-auto">
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 mb-3 transition-all duration-300 w-12 rounded-xl py-1 ${view === "diary" || view === "review" ? "text-blue-600 -translate-y-1 bg-blue-50" : "text-slate-400 hover:text-slate-500"}`}
          >
            <div className="relative">
              <BookOpen size={20} strokeWidth={view === "diary" || view === "review" ? 3 : 2} />
              {hasUnreadReview && (
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 mb-3 transition-all duration-300 w-12 rounded-xl py-1 ${view === "team_status" ? "text-blue-600 -translate-y-1 bg-blue-50" : "text-slate-400 hover:text-slate-500"}`}
          >
            <div className="relative">
              <Users size={20} strokeWidth={view === "team_status" ? 3 : 2} />
              {hasUnreadDiary && (
                <span className="absolute -top-1 -right-1 flex h-2.5 w-2.5">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-rose-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-rose-500 border border-white"></span>
      ...
      {isNotifOpen && (
        <div
          className="fixed inset-0 z-[120] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in"
          onClick={() => setIsNotifOpen(false)}
        >
          <div
            className="bg-white w-full max-w-md max-h-[80vh] rounded-[2.5rem] flex flex-col shadow-2xl animate-in zoom-in-95"
            onClick={(e) => e.stopPropagation()}
          >
            {/* ヘッダー */}
